    ...
    if (Serial.available()) {
        ...
        if (chartoInt(lrc, 3) == check && chartoInt(add, 2) == local) {
            ...
            if (charEqual(cmd, ini, 3)) {
                size = INI(measure);
            } else if (charEqual(cmd, tcp, 3)) {
                size = readLM35_A1(measure);
...
int readLM35_A1(char data_out[]) {
    ...
    float avgTemp = sum / 25.0 - 2.6; //CORREGIDO TEMP
    dtostrf(avgTemp, 7, 2, data_out);
    return strlen(data_out);
}
...
int readEmissivity(char data_out[]) {
    ...
    for (int i = 0; i < 25; i++) {
        ...
        sum_T_tcup += analogRead(lm35PinA1) * 5.0 * 100.0 / 1024.0 + 273.15;
        delay(40);
    }
