    ...
    if (chartoInt(lrc, 3) == check) { // Si el LRC calculado coincide con el LRC recibido:
      if (chartoInt(add, 2) == local) { // Si el mensaje está destinado al ID local:
        if (charEqual(cmd, ini, 3)) { // Si el comando es "INI":
          ...
          writeMsg(add, cmd, measure, size); // Envía la respuesta.
        } else if (charEqual(cmd, vol, 3)) { // Si el comando es "VOL":
          char measure[20] = {0}; // Define el espacio para almacenar la respuesta.
          int size = VOL(measure, data); // Llama a la función VOL para generar una respuesta.
          writeMsg(add, cmd, measure, size); // Envía la respuesta.
        } 
        else {
...
int VOL(char data_out[], char data_in[]) {
  
  //Extraer la ganancia del amplificador de instrumentacion. Se espera un valor de ganancia entero
  //Formato (xxxx)
  int gain =0;
  char gainStr[7];
  subchar(gainStr, data_in, 0, 6); // Extraer los primeros 6 caracteres como intensidad

  gain = chartoInt(gainStr, 6); // Convertir ganancia a entero
  // Aseguramos que la intensidad está en el rango permitido
  gain = constrain(gain, 1, 1000000);

  int analogValue = analogRead(analogPin); // Lee la entrada analogica (0 to 1023)
  double voltage = ((analogValue * (5.00 / 1023.00))/gain); // Conversion a voltaje (0 to 5V)


  String voltage_string = String(voltage,4);
  int length = voltage_string.length()+1;
  char buffer[length]={0};
  voltage_string.toCharArray(buffer, length);
  subchar(data_out, buffer, 0, length);
